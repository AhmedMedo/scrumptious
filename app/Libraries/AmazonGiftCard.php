...
class AmazonGiftCard
{
    ...
    const ENDPOINT  ='agcod-v2-eu-gamma.amazon.com';

    public function __construct()
    {

        $this->secret_key = env('GIFT_CARD_SECRET');
		$this->partner_id=env('GIFT_CARD_PARTNER_ID');
		$this->access_key= env('GIFT_CARD_KEY');
		$this->debug = true;
     ...
     * Buy a gift cardva
     *
     * @param $gcRequestId  generated unique key, with partnerID as the prefix.  Example: $partner_id . time()
     */
    public function buyGiftCard($gcRequestId, $gcValue=100.00)
    {
        $partnerId = $this->partner_id;
        $currency = 'AED';
        $serviceOperation = 'CreateGiftCard';

        $payload = $this->get_paypload_giftcard($partnerId, $gcRequestId, $currency, floatval($gcValue));
